...
public class ProcessingShapes extends PApplet {
	...
	static public void main(String args[]) {
		PApplet.main(new String[] { "--present", "--bgcolor=#000000",
				"--present-stop-color=#000000", "ProcessingShapes" });
	}

	/**
	 * This methode will be called from a super class or somewhere else...
	 * idk... Whatever: Do all the stuff tu setup your processing sketch here!
	 */
	@Override
	public void setup() {
	...
	private void setupKinect() {
		...
		System.out.println("kinectWidth:" + kinectWidth + "; kinectHeight:"
				+ kinectHeight);
		System.out.println(context.depthWidth() + ", " + context.depthHeight());
	...
	/**
	 * Another method that will be called automatically... Do all the drawing
	...
	public void draw() {
		...
		if (kinectIsConfigured)
			drawKinectStuff();

		if (soundIsConfigured)
			drawSoundLines();
		...
		for (int i = 1; i < 3; i++) {
			if (context.isTrackingSkeleton(i)) {
				...
				context.getJointPositionSkeleton(i,
						SimpleOpenNI.SKEL_RIGHT_HAND, position);
				drawColoredShapeWithForms(shapes.get(i - 1),
						getColorOfJoint(i, SimpleOpenNI.SKEL_RIGHT_HAND),
						position);
//				drawColoredShape(shapes.get(i + 1),
//						getColorOfJoint(i, SimpleOpenNI.SKEL_LEFT_HAND));
//				drawColoredShape(shapes.get(i + 3),
//						getColorOfJoint(i, SimpleOpenNI.SKEL_RIGHT_KNEE));
//				drawColoredShape(shapes.get(i + 5),
//						getColorOfJoint(i, SimpleOpenNI.SKEL_LEFT_KNEE));
//				drawColoredShape(shapes.get(i + 7),
//						getColorOfJoint(i, SimpleOpenNI.SKEL_HEAD));
	...
	private void drawSoundLines() {
		...
		for (int i = 0; i < ai.bufferSize() - 1; i++) {
			...
			// line(i, 150 + ai.right.get(i) * 50, i + 1, 150 + ai.right.get(i +
			// 1) * 50);
		}
	}
	 ...
	 * @param color
	 */
	private void drawColoredShapeWithForms(Shape shape, Color color,
			PVector jointPos) {
		...
		float x = jointPos.x * shape.getBoundingBox().getWidth()
				/ kinectWidth;
		float y = jointPos.y * shape.getBoundingBox().getHeight()
				/ kinectHeight;
	...
	/**
	 * This method draws a shape and fill it with an color !!!!!!Funzt grad
	 * nicht !!!!!!Funktioniert nur ohne translate...dh die Punkte in Shape
	 * mŸssen aufs ganze Koordinatensystem umgerechnet werden. 1
	 * 
	 * @param shape
	 */
	private void drawTexturedShape(Shape shape) {
		...
		ellipse(shape.getBoundingBox().getLeftTop().x, shape.getBoundingBox()
				.getLeftTop().y, 10, 10);
		fill(Color.yellow.getRGB());
		ellipse(shape.getBoundingBox().getRightBottom().x, shape
				.getBoundingBox().getRightBottom().y, 10, 10);
		fill(Color.red.getRGB());
		ellipse(shape.getBoundingBox().getMidPoint().x, shape.getBoundingBox()
				.getMidPoint().y, 10, 10);
		popMatrix();
	}
